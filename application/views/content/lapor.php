...
<!-- Z-INDEX 0 PENTING BIAR GA NIMPAL NAVBAR -->
...
<div class="max-w-7xl mx-auto px-6 py-16">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-16"> <!-- GAP DIPERBESAR JADI 16 -->
            ...
            <div class="sticky top-28">
                <div class="bg-brand-dark border border-gray-800 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
                    ...
                    <?= form_open_multipart('web/lapor_submit', ['class' => 'space-y-4 relative z-10']) ?>
                        ...
                        <div class="space-y-1"><label class="text-[10px] uppercase font-bold text-gray-500">Foto Bukti</label><div class="relative group cursor-pointer"><input type="file" name="image_before" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" required><div class="bg-black/50 border border-dashed border-gray-600 rounded-lg p-3 text-center group-hover:border-brand-green transition"><span class="text-xs text-gray-400 group-hover:text-white"><i class="fas fa-camera mr-1"></i> Klik Upload Foto</span></div></div></div>
...
<div id="galleryModal" class="fixed inset-0 bg-black/95 z-[9999] hidden flex items-center justify-center p-4 backdrop-blur-xl opacity-0 transition-opacity duration-300">
    <div class="w-full max-w-4xl bg-brand-dark rounded-2xl border border-gray-700 shadow-2xl overflow-hidden relative transform scale-95 transition-transform duration-300" id="modalContent">
        <div class="p-6 border-b border-gray-800 flex justify-between items-center bg-black/30"><div><h3 class="text-2xl font-heading font-bold text-white uppercase" id="modalTitle"></h3><p class="text-sm text-brand-green flex items-center gap-2"><i class="fas fa-check-circle"></i> Dibersihkan pada: <span id="modalDate" class="text-white"></span></p></div><button onclick="closeGalleryModal()" class="w-10 h-10 rounded-full bg-gray-800 text-gray-400 hover:text-white transition flex items-center justify-center"><i class="fas fa-times text-lg"></i></button></div>
        <div class="p-6 bg-black relative">
            <div class="ba-slider border-2 border-gray-800 shadow-2xl" onmousemove="geserSlider(event, this)" ontouchmove="geserSlider(event, this)">
                <img id="modalImgAfter" src="" alt="Sesudah" draggable="false"><div class="ba-before"><img id="modalImgBefore" src="" alt="Sebelum" draggable="false"></div><div class="absolute top-4 left-4 z-20 bg-black/60 text-white text-[10px] font-bold px-3 py-1 rounded">BEFORE</div><div class="absolute top-4 right-4 z-20 bg-brand-green/80 text-black text-[10px] font-bold px-3 py-1 rounded">AFTER</div><div class="ba-handle"><div class="ba-circle"><i class="fas fa-arrows-left-right"></i></div></div>
            </div>
...
<script>
    ...
    function placeMarker(lat, lng) { if (userMarker) { userMarker.setLatLng([lat, lng]); } else { userMarker = L.marker([lat, lng], { draggable: true }).addTo(map); userMarker.on('dragend', function(e) { const pos = e.target.getLatLng(); updateInputCoords(pos.lat, pos.lng); map.panTo(pos); }); userMarker.bindPopup("<b>Lokasi Laporan</b><br>Geser untuk sesuaikan.").openPopup(); } updateInputCoords(lat, lng); map.panTo([lat, lng]); }
    ...
    db_laporan.forEach(r => { let warna = r.status === 'pending' ? '#ef4444' : (r.status === 'in_progress' ? '#f59e0b' : '#10b981'); const customIcon = L.divIcon({ html: `<div style="background-color: ${warna}; width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 10px ${warna};"></div>`, className: 'custom-marker-icon' }); L.marker([r.latitude, r.longitude], { icon: customIcon }).addTo(map).bindPopup(`<div style="color:#333; font-family:sans-serif; min-width:150px;"><b style="text-transform:uppercase; font-size:11px;">${r.location_address}</b><br><span style="font-size:10px; font-weight:bold; color:${warna}; text-transform:uppercase;">${r.status.replace('_', ' ')}</span></div>`); });

    // FIX SORTING & LOAD MORE
    function sortGallery(type) {
        ...
        if (type === 'latest') {
            btnLatest.classList.replace('text-gray-500', 'text-white'); btnLatest.classList.replace('bg-transparent', 'bg-gray-800'); btnLatest.classList.add('bg-gray-800');
        ...
        // Reset Visibility: Tampilkan 6 awal, sembunyikan sisa
        const limit = 6;
    ...
    function closeGalleryModal() { const modal = document.getElementById('galleryModal'); const content = document.getElementById('modalContent'); modal.classList.add('opacity-0'); content.classList.remove('scale-100'); content.classList.add('scale-95'); setTimeout(() => { modal.classList.add('hidden'); }, 300); }
    function geserSlider(e, element) { const rect = element.getBoundingClientRect(); const x = (e.touches ? e.touches[0].clientX : e.clientX) - rect.left; let percent = Math.max(0, Math.min(100, (x / rect.width) * 100)); element.querySelector('.ba-before').style.clipPath = `inset(0 ${100 - percent}% 0 0)`; element.querySelector('.ba-handle').style.left = `${percent}%`; }
</script>
